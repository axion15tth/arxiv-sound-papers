...
const PaperCard = ({ paper, isBookmarked, paperTags = [] }) => {
  return (
    <div className="card group">
      {/* タイトル */}
      <div className="mb-1">
        <h3 className="text-base sm:text-lg md:text-xl font-bold mb-2 transition-colors" style={{ color: '#ffffff' }}>
          {paper.titleJa || paper.title}
        </h3>
        {paper.titleJa && (
          <>
            <div className="border-b border-primary-700/70 mb-2"></div>
            <p className="text-[10px] sm:text-xs md:text-sm italic font-mono bg-gradient-to-r from-primary-300 via-primary-200 to-primary-300 bg-clip-text text-transparent">
              {paper.title}
            </p>
          </>
        )}
      ...
      {/* メタ情報とアクション（1行） */}
      <div className="flex items-center justify-between gap-2 sm:gap-4 text-[10px] sm:text-xs md:text-sm" style={{ color: 'rgba(255, 255, 255, 0.65)' }}>
